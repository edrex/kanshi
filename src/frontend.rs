...
pub trait Frontend {
	fn apply_configuration<'a>(&self, Option<&'a Vec<MatchedOutput<'a>>>) -> Result<(), Box<Error>>;
}
...
impl SwayFrontend {
	...
	fn get_commands<'a>(&self, config: Option<&'a Vec<MatchedOutput<'a>>>) -> Vec<String> {
	...
	fn print_configuration<'a>(&self, config: Option<&'a Vec<MatchedOutput<'a>>>) -> Result<(), Box<Error>> {
...
impl Frontend for SwayFrontend {
	fn apply_configuration<'a>(&self, config: Option<&'a Vec<MatchedOutput<'a>>>) -> Result<(), Box<Error>> {
		let cmds = self.get_commands(config);
